      ...
      <AnimatePresence mode="wait">
        {tab === 'stats' ? (
          ...
          >
            ...
            {leaderboard.length > 0 && score > 0 && (
              <motion.div
                ...
                className="mt-1 p-3 rounded-xl bg-gradient-to-r from-easter-purple/10 to-easter-pink/10 border border-easter-purple/20 text-center"
              >
                ...
                {gap > 0 && (
                  <p className="text-[10px] text-gray-400 mt-0.5">{gap} pts behind #1</p>
                )}
                {gap === 0 && score > 0 && (
                  <p className="text-[10px] text-yellow-500 font-semibold mt-0.5">🏆 You're #1!</p>
                )}
              </motion.div>
            )}
          </motion.div>
          ...
          >
                  ...
                  <tbody>
                    {leaderboard.slice(0, 10).map((entry, i) => (
                      <tr
                        ...
                        } ${entry.score === score && score > 0 ? 'ring-1 ring-inset ring-easter-purple/40' : ''}`}
                      >
                        <td className="px-2 py-2">
                          {i < 3
                            ? <FaCrown className={`w-3 h-3 ${rankColor(i)}`} />
            ...
            <Link
              to="/leaderboard"
              className="mt-2 flex items-center justify-center gap-1.5 text-xs text-easter-purple dark:text-easter-pink hover:underline font-semibold py-1"
            >
